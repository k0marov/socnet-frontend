...
class PostCreationBloc extends Bloc<PostCreationEvent, PostCreationState> {
  ...
  PostCreationBloc(this._createPost) : super(const DefaultCreationState(images: [], currentSavedText: "")) {
    on<PostCreationEvent>((event, emit) async {
      ...
      if (event is ImageAdded) {
        ...
        final newImages = currentState.images.where((image) => image != event.deletedImage).toList();
        emit(DefaultCreationState(
          images: newImages,
          currentSavedText: currentState.currentSavedText,
        ));
      } else if (event is PostButtonPressed) {
        final newPostValue = NewPostValue(images: currentState.images, text: event.finalText);
        final result = await _createPost(PostCreateParams(newPost: newPostValue));
        result.fold(
