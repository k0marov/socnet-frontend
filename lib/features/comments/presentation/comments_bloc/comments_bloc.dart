  ...
  CommentsBloc(
    ...
    on<CommentsEvent>((event, emit) async {
      ...
      if (event is GetComments) {
        ...
        final resultEither =
            await _getPostComments(PostParams(post: event.post));
        resultEither.fold(
          (failure) => emit(CommentsLoadingFailure(failure: failure)),
          (comments) =>
              emit(CommentsLoaded(post: event.post, comments: comments)),
        );
      } else if (event is CommentAdded) {
        if (currentState is! CommentsLoaded) return;
        final resultEither = await _addComment(AddCommentParams(
            post: currentState.post, newComment: event.newComment));
        resultEither.fold(
        ...
        if (currentState is! CommentsLoaded) return;
        final resultEither =
            await _deleteComment(CommentParams(comment: event.deletedComment));
        resultEither.fold(
          (failure) => emit(CommentsLoadedFailure(
            failure: failure,
            post: currentState.post,
          ...
          (success) {
            final newComments = currentState.comments
                .where((comment) => comment != event.deletedComment)
                .toList();
            emit(CommentsLoaded(
              post: currentState.post,
        ...
        if (currentState is! CommentsLoaded) return;
        final resultEither = await _toggleLikeOnComment(
            CommentParams(comment: event.targetComment));
        resultEither.fold(
          (failure) => emit(CommentsLoadedFailure(
            failure: failure,
            post: currentState.post,
          ...
          (likedComment) {
            final newComments = currentState.comments
                .map<Comment>((comment) =>
                    comment == event.targetComment ? likedComment : comment)
                .toList();
            emit(CommentsLoaded(
              post: currentState.post,
              comments: newComments,
