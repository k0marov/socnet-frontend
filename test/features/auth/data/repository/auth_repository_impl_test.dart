...
void main() {
  ...
  setUp(() {
    ...
    sut =
        AuthRepositoryImpl(mockLocalTokenDataSource, mockNetworkAuthDataSource);
  });
  ...
  group('getToken', () {
    test(
      ...
      () async {
        ...
        when(() => mockLocalTokenDataSource.getToken())
  ...
  void sharedLoginAndRegister({required bool isLogin}) {
    ...
    test(
      ...
      () async {
        ...
        when(() => mockLocalTokenDataSource.storeToken(tTokenModel))
            .thenAnswer((_) async {});
        // act
    ...
    test(
      ...
      () async {
        ...
        if (isLogin) {
          when(() => mockNetworkAuthDataSource.login(tUsername, tPassword))
              .thenThrow(const NetworkException(4242, "some detail"));
        } else {
          when(() => mockNetworkAuthDataSource.register(tUsername, tPassword))
              .thenThrow(const NetworkException(4242, "some detail"));
        }
        ...
        final result = isLogin
            ...
            : await sut.register(tUsername, tPassword);
        // assert
        expect(result, const Left(NetworkFailure(4242, "some detail")));
      },
    );
  ...
  group('logout', () {
    test(
      ...
      () async {
        // arrange
        when(() => mockLocalTokenDataSource.deleteToken())
            .thenAnswer((_) async {});
        // act
