...
void main() {
  ...
  group('getToken', () {
    ...
    test(
      ...
      () async {
        ...
        // assert
        expect(
            () => sut.getToken(), throwsA(const TypeMatcher<CacheException>()));
      },
    );
    test(
      ...
      () async {
        // arrange
        when(() => mockSharedPreferences.getString(tokenCacheKey))
            .thenReturn("not-a-valid-token-json");
        // assert
        expect(
            () => sut.getToken(), throwsA(const TypeMatcher<CacheException>()));
      },
  ...
  group('storeToken', () {
    ...
    test(
      ...
      () async {
        ...
        when(() => mockSharedPreferences.setString(
            tokenCacheKey, json.encode(tJson))).thenAnswer((_) async => true);
        // act
        await sut.storeToken(tToken);
        // assert
        verify(() =>
            mockSharedPreferences.setString(tokenCacheKey, json.encode(tJson)));
        verifyNoMoreInteractions(mockSharedPreferences);
      },
    );
    test(
      ...
      () async {
        // arrange
        when(() => mockSharedPreferences.setString(
            tokenCacheKey, json.encode(tJson))).thenAnswer((_) async => false);
        // assert
        expect(() => sut.storeToken(tToken),
    ...
    test(
      ...
      () async {
        // arrange
        when(() => mockSharedPreferences.setString(
            tokenCacheKey, json.encode(tJson))).thenThrow(Exception());
        // assert
        expect(() => sut.storeToken(tToken),
            throwsA(const TypeMatcher<CacheException>()));
      },
  ...
  group('deleteToken', () {
    test(
      ...
      () async {
        // arrange
        when(() => mockSharedPreferences.remove(tokenCacheKey))
